/*
 * Base de Datos Flotilla · Integración Central de Cuentas
 *
 * Sincroniza únicamente el fondeo asignado desde Flotilla hacia las cuentas
 * compartidas de Central de Cuentas. El token privado se guarda en Script
 * Properties como CENTRAL_API_TOKEN; nunca en el HTML ni en Google Sheets.
 *
 * Requiere en Base de datos flotilla:
 *   - Config: CENTRAL_API_URL, CENTRAL_SYNC_ENABLED
 *   - Central_Map
 *   - Central_Sync
 */

var _centralBaseMarkPayment_ = markPayment_;
markPayment_ = function(body, device) {
  var result = _centralBaseMarkPayment_(body, device);
  if (result && result.ok) syncCentralCurrentWeekSafe_(device);
  return result;
};

var _centralBaseUnmarkPayment_ = unmarkPayment_;
...
var _centralBaseSaveUberWeek_ = saveUberWeek_;
...
function syncCentralCurrentWeekNow() {
  return syncCentralCurrentWeek_('manual_apps_script');
}

function syncCentralCurrentWeekSafe_(device) {
  try {
    return syncCentralCurrentWeek_(device || 'system');
  } catch (err) {
    log_('CENTRAL_SYNC', 'current_week', 'SYNC_ERROR', String(err && err.message || err), device || 'system');
    return {ok:false, message:String(err && err.message || err)};
  }
}

function syncCentralCurrentWeek_(device) {
  if (String(centralConfigValue_('CENTRAL_SYNC_ENABLED', 'FALSE')).toUpperCase() !== 'TRUE') {
    return {ok:true, disabled:true};
  }

  var token = props_().getProperty('CENTRAL_API_TOKEN');
  if (!token) {
    log_('CENTRAL_SYNC', 'current_week', 'NOT_CONFIGURED', 'Falta CENTRAL_API_TOKEN en Script Properties', device);
    return {ok:true, configured:false, message:'Falta CENTRAL_API_TOKEN en Script Properties.'};
  }

  var weekDate = centralCurrentTuesday_();
  var model = centralBuildCurrentWeekModel_(weekDate);
  var mappings = centralActiveMappings_();
  var desired = centralDesiredItems_(model, mappings);
  if (!desired.length) return {ok:true, configured:true, weekDate:weekDate, results:[]};
  ...
  desired.forEach(function(item) {
    ...
    if (!account && String(map.CuentaID || '') === 'u') {
    ...
    var currentBalance = num_(account.balance);
    var nextBalance = centralRound_(currentBalance + delta);
    if (nextBalance < -0.01) {
  ...
  return {ok:true, configured:true, weekDate:weekDate, results:results};
}

function centralBuildCurrentWeekModel_(weekDate) {
  ...
  var weekEnd = new Date(weekStart.getTime()); weekEnd.setUTCDate(weekEnd.getUTCDate() + 6);
  var rows = [];
  ...
  vehicles.forEach(function(v) {
    ...
    if (frequency === 'QUINCENAL_15_FIN_MES') {
      centralSpecialDatesForWindow_(v, weekStart, weekEnd).forEach(function(date) {
        rows.push({vehicleId:vehicleId,date:date,amount:num_(v.CuotaSemanal),iva:num_(v.CuotaSemanal)*0.13,insurance:centralDefaultInsurance_(v)});
      });
      return;
    }
    ...
    if (!start || !end) return;
    var d = new Date(start + 'T00:00:00Z'), finish = new Date(end + 'T00:00:00Z');
  ...
  var accounts = sheetObjects_(ss.getSheetByName('Cuentas')).filter(function(a) {
    var iso = weekDate, from = ymd_(a.VigenteDesde), to = ymd_(a.VigenteHasta);
    return (!from || iso >= from) && (!to || iso <= to) && String(a.Activa || '').toUpperCase() !== 'FALSE';
  }).map(function(a) {
    var id = String(a.CuentaID || ''), type = String(a.Tipo || 'SEMANAL').toUpperCase(), need = 0;
  ...
  if (rem) { rem.need = Math.max(0, remaining); rem.assigned = Math.max(0, remaining); }
...
function centralDesiredItems_(model, mappings) {
  ...
  ['omoda','coopealianza','u'].forEach(function(cuentaId) {
    ...
    var map = mappings.find(function(m){return String(m.CuentaID || '') === cuentaId && !String(m.VehicleID || '');});
    if (expense && map) out.push({
  ...
  model.rows.forEach(function(row) {
    var map = mappings.find(function(m){return String(m.CuentaID || '') === 'seguros' && String(m.VehicleID || '') === String(row.vehicleId);});
    if (!map || totalInsurance <= 0 || num_(row.insurance) <= 0) return;
    out.push({
...
function centralSpecialDatesForWindow_(vehicle, start, end) {
  var result = [], cursor = new Date(Date.UTC(start.getUTCFullYear(), start.getUTCMonth(), 1));
  ...
  while (cursor <= end) {
    ...
    [15,last].forEach(function(day) {
      var d = new Date(Date.UTC(y,m,day)), iso = Utilities.formatDate(d,'UTC','yyyy-MM-dd');
...
function centralConfigValue_(key, fallback) {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID), rows = sheetObjects_(ss.getSheetByName('Config'));
...
function centralSyncState_(weekDate, syncKey) {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID), sh = ss.getSheetByName('Central_Sync');
  if (!sh) return null;
  var key = String(weekDate) + '|' + String(syncKey);
  var rows = sheetObjects_(sh);
  return rows.find(function(r){return String(r.StateKey || '') === key;}) || null;
}

function centralUpsertSyncState_(weekDate, item, desired, synced, delta, status) {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID), sh = ss.getSheetByName('Central_Sync');
  if (!sh) return;
  var values = sh.getDataRange().getValues(), headers = values[0].map(String), idx = {};
  headers.forEach(function(h,i){idx[h]=i;});
  var key = String(weekDate) + '|' + String(item.syncKey), row = -1;
  for (var r=1;r<values.length;r++) if (String(values[r][idx.StateKey]) === key) {row=r+1;break;}
  var data = [key,weekDate,item.cuentaId,item.vehicleId,String(item.map.CentralAccountID || ''),desired,synced,delta,status,new Date()];
  if (row < 0) sh.appendRow(data); else sh.getRange(row,1,1,data.length).setValues([data]);
}
...
function centralApiGetAccounts_(token) {
  ...
  try {
    var response = UrlFetchApp.fetch(url + '?action=getAccounts&token=' + encodeURIComponent(token), {method:'get',muteHttpExceptions:true});
